...
export default class MyConsole extends Object {
    ...
    cursorPosition;
    history;
    currentlyActiveHistoryLine;
    firstTimeHistoryUsedFlag;
    textNode;
    console;
    cursorNode;
    consoleBuffer;
            ...
            case true:
                {
                    ...
                    // --> occurs if you change the direction during selection process
                        ...
                        //let coveredChar="";
                        if (offsetEnd === 0) {
                            this._coveredChar = CURSOR_END_CHAR;
                        } // Cursor is at the end, insert a space to make it visible.
                        else {
                            this._coveredChar = _buffer.slice(this.cursorPosition, this.cursorPosition + 1);
                        }
                        //Now get the text after the current selection.
                        ...
                        if (offsetEnd === 0) {
                            this._coveredChar = CURSOR_END_CHAR;
                        } // the cursor is at the end, insert a space the make it visible.
                        else {
                            this._coveredChar = _buffer.slice(this.cursorPosition, this.cursorPosition + 1);
                        }
    ...
    */
    printJsonData(_jsonData, _cssClassName = "") {
        ...
        // Dict
            ...
            _jsonData.keys().forEach((key) => this.printLine(_jsonData[key], _cssClassName));
        }
    }
    //
            ...
            case 'message': // Tests needed....
                {
                    ...
                    switch (e.origin) {
                        ...
                        case LOCAL_ADDR_SSL:
                            ...
                            {
                                switch (e.data.type) {
                                    case 1 /* MESSAGES.UPDATE */: // TODO: Plugins via worker threads need to trigger the update this way.
                                        {
                                    ...
                                    case true:
                                        {
                                            ...
                                            if (this.selectionStartPos > this.selectionEndPos) {
                                                this.currentSelection = this.consoleBuffer.slice(this.selectionEndPos, this.selectionStartPos + 1);
                                            } // +1 to get the end of text, selection is behind the cursor
                                            else {
                                                this.currentSelection = this.consoleBuffer.slice(this.selectionStartPos, this.selectionEndPos);
                                            } // +1 not necessary because selection is in front of cursor
    ...
    _clearConsoleComplete() {
        this._clearConsoleLineBuffer();
        let lines = document.querySelectorAll('.text:not(#text)'); // select all Elements with class = "text" and id != "text"
        for (let i = 0; i < lines.length; i++) {
            this.console.firstElementChild.remove();
        } // Later todo, find a better way to do this.
        this.lineCounter = 0;
    }
    //
            ...
            default:
                {
                    ...
                    if (textBeforeCursor.toLowerCase() === "cls") {
                        let msg = { type: 2 /* MESSAGES.CLS */, data: "" };
                    ...
                    else // everything else sent to loader class
                     {
                        ...
                        let msg = { type: 3 /* MESSAGES.SEND */, data: String(textBeforeCursor) };
