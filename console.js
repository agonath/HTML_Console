...
class MyConsole extends Object
{
	...
	//
	//	Event loop handler, all update go here...TODO
	//
	eventLoop()
	{
		...
		if(t > FPS && this.updateReqFlag === true)
		{
			...
			this.updateCursorLine(this._textNode, this.consoleBuffer, this.cursorPosition);
		}
	...
	//				_buffer			-> The user input as string. (String)
	...
	_handleBackspace(_e, _textNode)
	{
		if(_textNode.innerHTML != null && _textNode.innerHTML.length > 0)
		{
			...
			console.log("Textzeile nach Backspace: " + _textNode.innerHTML);
		}
		...
		switch(_e.keyCode)
		{
			case 37: // Left arrow key
			{
				...
				if(_textNode.innerHTML != null && this.cursorPosition > 0)
				{
					...
					//const i = window.getComputedStyle(_textNode);
					//const fontSize = parseInt(i.getPropertyValue("font-size"));		
					//let pos = String(this.cursorPosition * fontSize) + "px";
					//_textNode.style.setProperty("--cursorPosition", pos);

					console.log(this.cursorPosition);//.after.left = String(lineLength - fontSize) + "px";
					this.cursorPosition -= 1;
					this.updateReqFlag = true;
